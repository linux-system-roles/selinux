        ...
        if substitute is None:
            ...
            if exists:
                ...
                if substitute != orig_substitute:
                    ...
                    if module._diff:
                        prepared_diff += (
                            "# Change to semanage file context path substitutions\n"
                        )
                ...
                if module._diff:
                    prepared_diff += (
                        "# Addition to semanage file context path substitutions\n"
                    )
                    prepared_diff += "+%s = %s\n" % (target, substitute)
...
def semanage_fcontext_delete(
    module, result, target, ftype, setype, substitute, do_reload, sestore=""
):
    ...
    try:
        ...
        if (
            ...
            and setype is None
            and (
                (substitute is not None and substitute_exists == substitute)
                or substitute is None
            )
        ):
            ...
            if module._diff:
                prepared_diff += (
                    "# Deletion to semanage file context path substitutions\n"
                )
                prepared_diff += "-%s = %s\n" % (target, orig_substitute)
...
def main():
    ...
    result = dict(
        target=target, ftype=ftype, setype=setype, substitute=substitute, state=state
    )
    ...
    if state == "present":
        ...
        semanage_fcontext_delete(
            module, result, target, ftype, setype, substitute, do_reload
        )
    else:
